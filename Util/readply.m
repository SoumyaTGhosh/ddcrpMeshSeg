function [Pts Tri] = readply(raw_scan_name)

ply_data=readply_internal(raw_scan_name);
Tri=1+vertcat(ply_data.face.vertex_indices{:});
Pts=[ply_data.vertex.x(:)'; ply_data.vertex.y(:)'; ply_data.vertex.z(:)'];




function [Elements,varargout] = readply_internal(Path,Str)
